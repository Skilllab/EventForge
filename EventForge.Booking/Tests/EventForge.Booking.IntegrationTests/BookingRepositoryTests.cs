...
public class BookingRepositoryTests : BaseRepositoryTest
{
    ...
    [Fact]
    public async Task CancelAndAddOutboxAsync_Should_Update_Status_And_Create_Outbox_Record()
    {
        // Arrange
        await ResetDatabaseAsync();
        var repository = CreateRepository();
        var userId = Guid.NewGuid();
        var booking = BookingModel.Create(Guid.NewGuid(), userId, DateTime.UtcNow);
        await repository.AddAsync(booking, CancellationToken.None);
        var processedAt = DateTime.UtcNow;
        var outbox = OutboxMessage.Create(nameof(BookingCancelled), TopicNames.BookingCancelled, booking.EventId.ToString(), "payload", processedAt, null);

        // Act
        var updated = await repository.CancelAndAddOutboxAsync(booking.Id, userId, processedAt, outbox, CancellationToken.None);
        var result = await repository.GetByIdAsync(booking.Id, CancellationToken.None);
        await using var context = await CreateContext();
        var outboxCount = await context.OutboxMessages.CountAsync();
    ...
    [Fact]
    public async Task RejectAndAddOutboxAsync_Should_Update_Status_And_Create_Outbox_Record()
    {
        // Arrange
        await ResetDatabaseAsync();
        var repository = CreateRepository();
        ...
        await repository.AddAsync(booking, CancellationToken.None);
        var processedAt = DateTime.UtcNow;
        var outbox = OutboxMessage.Create(nameof(BookingRejected), TopicNames.BookingRejected, booking.EventId.ToString(), "payload", processedAt, null);

        // Act
        var updated = await repository.RejectAndAddOutboxAsync(booking.Id, processedAt, outbox, CancellationToken.None);
        var result = await repository.GetByIdAsync(booking.Id, CancellationToken.None);
        await using var context = await CreateContext();
        var outboxCount = await context.OutboxMessages.CountAsync();
